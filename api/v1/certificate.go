...
func (c certificateApi) Get(context echo.Context) error {
	...
	code, data := c.certificateService.Get(agent, ownerReference, processId, option)
	if code == 200 {
...
func (c certificateApi) GetByID(context echo.Context) error {
	...
	code, data := c.certificateService.GetByID(id, agent, processId)
	if code == 200 {
		return context.JSON(code, data)
	}
	return common.GenerateErrorResponse(context, "[ERROR]: k8s Object Query Failed", "Operation Failed")
...
//This function is for setting all query param to query K8SObjects
...
func NewCertificateApi(certificateService service.Certificate, jwtService service.Jwt, processService service.Process) api.Certificate {
