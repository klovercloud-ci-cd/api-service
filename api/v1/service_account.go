...
// Get... Get Api
// @Summary Get api
// @Description Api for getting all ServiceAccount by agent name, owner reference and process
// @Tags ServiceAccount
// @Produce json
...
func (s serviceAccountApi) Get(context echo.Context) error {
	...
	code, data := s.serviceAccountService.Get(agent, ownerReference, processId, option)
	if code == 200 {
...
// @Description Api for getting a ServiceAccount by id, agent name, and process id
// @Tags ServiceAccount
// @Produce json
...
func NewServiceAccountApi(serviceAccountService service.Service, jwtService service.Jwt, processService service.Process) api.ServiceAccount {
