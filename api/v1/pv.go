...
// Get... Get Api
// @Summary Get api
// @Description Api for getting all PersistentVolume by agent name, owner reference and process
...
func (p persistentVolumeApi) Get(context echo.Context) error {
	...
	code, data := p.persistentVolumeService.Get(agent, ownerReference, processId, option)
	if code == 200 {
...
// @Description Api for getting a PersistentVolume by id, agent name, and process id
// @Tags PersistentVolume
// @Produce json
...
func (p persistentVolumeApi) GetByID(context echo.Context) error {
	...
	code, data := p.persistentVolumeService.GetByID(id, agent, processId)
	if code == 200 {
		return context.JSON(code, data)
	}
	return common.GenerateErrorResponse(context, "[ERROR]: k8s Object Query Failed", "Operation Failed")
...
func NewPersistentVolumeApi(persistentVolumeService service.PersistentVolume, jwtService service.Jwt, processService service.Process) api.PersistentVolume {
