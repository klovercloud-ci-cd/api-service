...
// Get... Get Api
// @Summary Get api
// @Description Api for getting all ClusterRoleBinding by agent name, owner reference and process
...
func (c clusterRoleBindingApi) Get(context echo.Context) error {
	...
	code, data := c.clusterRoleBindingService.Get(agent, ownerReference, processId, option)
	if code == 200 {
...
func (c clusterRoleBindingApi) GetByID(context echo.Context) error {
	...
	code, data := c.clusterRoleBindingService.GetByID(id, agent, processId)
	if code == 200 {
		return context.JSON(code, data)
	}
	return common.GenerateErrorResponse(context, "[ERROR]: k8s Object Query Failed", "Operation Failed")
...
func NewClusterRoleBindingApi(clusterRoleBindingService service.ClusterRoleBinding, jwtService service.Jwt, processService service.Process) api.ClusterRoleBinding {
