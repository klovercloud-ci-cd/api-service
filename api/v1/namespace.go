...
// Get... Get Api
// @Summary Get api
// @Description Api for getting all Namespace by agent name, owner reference and process
// @Tags Namespace
// @Produce json
...
func (n namespaceApi) Get(context echo.Context) error {
	...
	code, data := n.namespaceService.Get(agent, ownerReference, processId, option)
	if code == 200 {
...
func (n namespaceApi) GetByID(context echo.Context) error {
	...
	code, data := n.namespaceService.GetByID(id, agent, processId)
	if code == 200 {
		return context.JSON(code, data)
	}
	return common.GenerateErrorResponse(context, "[ERROR]: k8s Object Query Failed", "Operation Failed")
...
func NewNamespaceApi(namespaceService service.Namespace, jwtService service.Jwt, processService service.Process) api.Namespace {
	return &namespaceApi{
