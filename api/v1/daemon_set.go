...
// Get... Get Api
// @Summary Get api
// @Description Api for getting all DaemonSet by agent name, owner reference and process
// @Tags DaemonSet
// @Produce json
...
func (d daemonSetApi) Get(context echo.Context) error {
	...
	code, data := d.daemonSetService.Get(agent, ownerReference, processId, option)
	if code == 200 {
...
func (d daemonSetApi) GetByID(context echo.Context) error {
	...
	code, data := d.daemonSetService.GetByID(id, agent, processId)
	if code == 200 {
		return context.JSON(code, data)
	}
	return common.GenerateErrorResponse(context, "[ERROR]: k8s Object Query Failed", "Operation Failed")
...
func NewDaemonSetApi(daemonSetService service.DaemonSet, jwtService service.Jwt, processService service.Process) api.DaemonSet {
	return &daemonSetApi{
