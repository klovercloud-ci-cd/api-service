...
// Get... Get Api
// @Summary Get api
// @Description Api for getting all PersistentVolumeClaim by agent name, owner reference and process
...
func (p persistentVolumeClaimApi) Get(context echo.Context) error {
	...
	code, data := p.persistentVolumeClaimService.Get(agent, ownerReference, processId, option)
	if code == 200 {
...
// @Description Api for getting a PersistentVolumeClaim by id, agent name, and process id
// @Tags PersistentVolumeClaim
// @Produce json
...
func (p persistentVolumeClaimApi) GetByID(context echo.Context) error {
	...
	code, data := p.persistentVolumeClaimService.GetByID(id, agent, processId)
	if code == 200 {
		return context.JSON(code, data)
	}
	return common.GenerateErrorResponse(context, "[ERROR]: k8s Object Query Failed", "Operation Failed")
...
func NewPersistentVolumeClaimApi(persistentVolumeClaimService service.PersistentVolumeClaim, jwtService service.Jwt, processService service.Process) api.PersistentVolumeClaim {
