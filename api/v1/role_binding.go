...
// Get... Get Api
// @Summary Get api
// @Description Api for getting all RoleBinding by agent name, owner reference and process
// @Tags RoleBinding
// @Produce json
...
func (r roleBindingApi) Get(context echo.Context) error {
	...
	code, jsonBody := r.processService.GetById(companyId, processId)
	if code != 200 {
	...
	code, data := r.roleBindingService.Get(agent, ownerReference, processId, option)
	if code == 200 {
...
// @Description Api for getting a RoleBinding by id, agent name, and process id
// @Tags RoleBinding
// @Produce json
...
func (r roleBindingApi) GetByID(context echo.Context) error {
	...
	code, data := r.roleBindingService.GetByID(id, agent, processId)
	if code == 200 {
		return context.JSON(code, data)
	}
	return common.GenerateErrorResponse(context, "[ERROR]: k8s Object Query Failed", "Operation Failed")
...
func NewRoleBindingApi(roleBindingService service.RoleBinding, jwtService service.Jwt, processService service.Process) api.RoleBinding {
