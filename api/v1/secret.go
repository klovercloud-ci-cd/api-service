...
// Get... Get Api
// @Summary Get api
// @Description Api for getting all Secret by agent name, owner reference and process
// @Tags Secret
// @Produce json
...
func (s secretApi) Get(context echo.Context) error {
	...
	code, data := s.seceretService.Get(agent, ownerReference, processId, option)
	if code == 200 {
...
func NewSecretApi(seceretService service.Secret, jwtService service.Jwt, processService service.Process) api.Secret {
	return &secretApi{
		seceretService: seceretService,
