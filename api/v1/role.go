...
// Get... Get Api
// @Summary Get api
// @Description Api for getting all Role by agent name, owner reference and process
// @Tags Role
// @Produce json
...
func (r roleApi) Get(context echo.Context) error {
	...
	code, data := r.roleService.Get(agent, ownerReference, processId, option)
	if code == 200 {
...
func (r roleApi) GetByID(context echo.Context) error {
	...
	code, data := r.roleService.GetByID(id, agent, processId)
	if code == 200 {
		return context.JSON(code, data)
	}
	return common.GenerateErrorResponse(context, "[ERROR]: k8s Object Query Failed", "Operation Failed")
...
func NewRoleApi(roleService service.Role, jwtService service.Jwt, processService service.Process) api.Role {
	return &roleApi{
