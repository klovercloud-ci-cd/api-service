...
// Get... Get Api
// @Summary Get api
// @Description Api for getting all NetworkPolicy by agent name, owner reference and process
// @Tags NetworkPolicy
// @Produce json
...
func (n networkPolicyApi) Get(context echo.Context) error {
	...
	code, data := n.networkPolicyService.Get(agent, ownerReference, processId, option)
	if code == 200 {
...
func (n networkPolicyApi) GetByID(context echo.Context) error {
	...
	code, data := n.networkPolicyService.GetByID(id, agent, processId)
	if code == 200 {
		return context.JSON(code, data)
	}
	return common.GenerateErrorResponse(context, "[ERROR]: k8s Object Query Failed", "Operation Failed")
...
func NewNetworkPolicyApi(networkPolicyService service.NetworkPolicy, jwtService service.Jwt, processService service.Process) api.NetworkPolicy {
